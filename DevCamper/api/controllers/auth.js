...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  // Get reset token, return an unhashed token and this is what we send it as email
  const resetToken = user.getResetPasswordToken();

  await user.save({ validateBeforeSave: false });

  // Create reset url
  const resetURL = `${req.protocol}://${req.get('host')}${process.env.AUTH_API_PATH}/resetpassword/${resetToken}`;

  const text = `You are receiving this email because you (or someone else) has ` +
...
exports.resetPassword = asyncHandler(async (req, res, next) => {
  ...
  const user = await User.findOne({
    resetPasswordToken,
    resetPasswordExpire: {
      $gt: Date.now() /* greater than current date */
    }
  });
...
// Get token from model, create cookie and send response, this makes the user logged-in
